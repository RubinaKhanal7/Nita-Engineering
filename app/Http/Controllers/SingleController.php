...
class SingleController extends Controller
{
   ...
   public function render_projects(Request $request)
    ...
    public function render_single_project($slug)

    {
        ...
        $remainingproject = Project::all()->except($project->slug);
        $posts = Post::with('get_categories')->latest()->get()->take(3);
        $categorieson = Category::with(['get_posts'=>function($query){
            $query->latest()->take(8);
        }])->whereIn('id',[1])->get();
        ...
        return view('portal.singleproject', compact('sitesetting','mvcs','coverimage','posts','teams','categories','categorieson','categoriesone','abouts','project', 'services', 'remainingproject'));
    }
    ...
    public function single_image(Gallery $gallery)
    {
        $categories = Category::latest()->get()->take(10);
        $mvcs = Mvc::latest()->get()->take(3);
        $coverimage= CoverImage::latest()->get()->take(4);
        ...
        return view('portal.singleimage', compact('gallery','sitesetting','mvcs','coverimage','posts','teams','categories','categorieson','categoriesone','abouts','services',));
    }
